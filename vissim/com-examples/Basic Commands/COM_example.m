...
% This scrips demonstrates how to use the COM interface in MATLAB. 
...
Vissim = actxserver('Vissim.Vissim-64.700'); 
...
% Vissim = actxserver('Vissim.Vissim-32.600'); % Vissim 6 - 32 bit 
% Vissim = actxserver('Vissim.Vissim-64.600'); % Vissim 6 - 64 bit 
% Vissim = actxserver('Vissim.Vissim-32.700'); % Vissim 7 - 32 bit 
% Vissim = actxserver('Vissim.Vissim-64.700'); % Vissim 7 - 64 bit 

Path_of_COM_example_network = cd; %'C:\Users\Public\Documents\PTV Vision\PTV Vissim 7\Examples Training\COM\Basic Commands';

%% Load a Vissim Network:
Filename                = fullfile(Path_of_COM_example_network, 'COM_example.inpx');
flag_read_additionally  = false; % you can read network(elements) additionally, in this case set "flag_read_additionally" to true
Vissim.LoadNet(Filename, flag_read_additionally);

%% Load a Layout:
Filename = fullfile(Path_of_COM_example_network, 'COM_example.layx');
...
% Name_of_Link = Vissim.Net.Links.ItemByKey(Link_number).AttValue('Name'); % alternativ (using the method "AttValue")
...
set(Rel_Flows{1}, 'AttValue', 'DesSpeedDistr',   50); % Changing the disired speeed distribution
...
% GetMultipleAttributes     Read multiple attributes of all objects: --does not work in MATLAB
%Attributes1 = {'Name', 'Length2D'};
%Name_Length_of_Links = Vissim.Net.Links.GetMultipleAttributes(Attributes1);

% SetMultipleAttributes     Set multiple attribute of multiple (always the first x) objects: --does not work in MATLAB
...
State_of_SH = get(Vissim.Net.SignalHeads.ItemByKey(SH_number), 'AttValue', 'State'); % possible output: 'GREEN', 'RED', 'AMBER', 'REDAMBER'
...
set(SignalGroup, 'AttValue', 'State', new_state);
...
% Vissim.LoadLayout(fullfile(Path_of_COM_example_network, 'COM_example_without_vehicles.layx')); % loading a layout where vehicles are not displayed => much faster simulation
...
% Vissim.LoadLayout(fullfile(Path_of_COM_example_network, 'COM_example.layx')); % loading a layout to display vehicles again
...
disp(['Average travel time all time intervalls of all simulation of all vehicle classes:',32,num2str(TT),32,'(number of vehicles:',32,num2str(No_Veh),')']) % char(32) is whitespace
...
disp(['Maximum travel time of all time intervalls of the current simulation of vehicle class HGV:',32,num2str(TT),32,'(number of vehicles:',32,num2str(No_Veh),')']) % char(32) is whitespace
...
disp(['Data Collection #',num2str(DC_measurement_number),': Average values of all Simulations runs of 1st time intervall of all vehicle classes:'])
...
% Saving
%==========================================================================
Filename = fullfile(Path_of_COM_example_network, 'COM_example_saved.inpx');
Vissim.SaveNetAs(Filename)
Filename = fullfile(Path_of_COM_example_network, 'COM_example_saved.layx');
