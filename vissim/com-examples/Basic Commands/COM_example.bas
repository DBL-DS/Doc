...
' This scrips demonstrates how to use the COM interface in VBA.
...
' languages Matlab, Python, C# and C++.
...
Set Vissim = CreateObject("Vissim.Vissim-64.700")
...
' Set Vissim = CreateObject("Vissim.Vissim-32.600") ' Vissim 6 - 32 bit
' Set Vissim = CreateObject("Vissim.Vissim-64.600") ' Vissim 6 - 64 bit
' Set Vissim = CreateObject("Vissim.Vissim-32.700") ' Vissim 7 - 32 bit
' Set Vissim = CreateObject("Vissim.Vissim-64.700") ' Vissim 7 - 64 bit

Dim Path_of_COM_example_network As String
...
Filename = Path_of_COM_example_network & "\COM_example.inpx"
...
Rel_Flows(0).AttValue("DesSpeedDistr") = 50 ' Changing the disired speeed distribution
...
Dim Name_of_Links As Variant
Attribute1 = "Name"
Name_of_Links = Vissim.Net.Links.GetMultiAttValues(Attribute1)

' SetMultiAttValues         Set one attribute of multiple (not necessarily all) objects
Dim Link_No_Name(2, 1) As Variant
Link_No_Name(0, 0) = 1
Link_No_Name(0, 1) = "New Link Name of Link #1"
Link_No_Name(1, 0) = 2
Link_No_Name(1, 1) = "New Link Name of Link #2"
Link_No_Name(2, 0) = 4
Link_No_Name(2, 1) = "New Link Name of Link #4"
Vissim.Net.Links.SetMultiAttValues Attribute1, Link_No_Name ' 1st input is the Link number, 2nd the link name
...
SignalGroup.AttValue("State") = new_state
...
' Vissim.LoadLayout Path_of_COM_example_network & "\COM_example.layx"
...
cnt_row = cnt_row + 1
Cells(cnt_row, 1) = "Average travel time all time intervalls of all simulation of all vehicle classes: " & TT & " number of vehicles: " & No_Veh & ")"    ' output to Excel
...
cnt_row = cnt_row + 1
Cells(cnt_row, 1) = "Maximum travel time of all time intervalls of the current simulation of vehicle class HGV: " & TT & " number of vehicles: " & No_Veh & ")"    ' output to Excel
...
cnt_row = cnt_row + 1
Cells(cnt_row, 1) = "Data Collection #" & DC_measurement_number & ": Average values of all Simulations runs of 1st time intervall of all vehicle classes"    ' output to Excel
